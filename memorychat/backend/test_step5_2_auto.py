#!/usr/bin/env python3
"""
Automated test script for Step 5.2: API Endpoints
Tests all endpoints without requiring user input.
"""
import sys
import os
...
def test_chat_endpoints(session_id: Optional[int]):
    ...
    # Test POST /api/chat/message (may fail if OpenAI API key not set, but should handle gracefully)
    ...
    if result and "error" not in result:
        if result["status_code"] == 200 and result["data"]:
            ...
            # Might fail if OpenAI API key not configured - that's OK for structure test
            print_test("POST /api/chat/message", True, "Endpoint exists (may need API key)")
        else:
            print_test("POST /api/chat/message", False, f"Status: {result.get('status_code')}")
    else:
    ...
    if result2 and result2["status_code"] == 200:
        ...
        print_test("GET /api/sessions/{session_id}/messages", False, str(result2))
    ...
    if result3 and result3["status_code"] == 200:
        ...
        print_test("GET /api/sessions/{session_id}/context", False, str(result3))
...
def test_memory_endpoints(profile_id: Optional[int]):
    ...
    if result and result["status_code"] == 200:
        ...
        print_test("GET /api/profiles/{profile_id}/memories", False, str(result))
    
    # Test POST /api/memories/search
    print("\n2. Testing POST /api/memories/search")
    # Note: This endpoint uses Query parameters, so we need to pass them in the URL
    search_url = f"/memories/search?profile_id={profile_id}&query=test&limit=10"
    result2 = make_request("POST", search_url)
    if result2 and result2["status_code"] == 200:
        print_test("POST /api/memories/search", True)
    else:
        print_test("POST /api/memories/search", False, str(result2))
...
def test_validation():
    ...
    if result2 and result2["status_code"] == 422:
        ...
        print_test("Empty username validation", False, f"Expected 422, got {result2.get('status_code') if result2 else 'None'}")
...
def main():
    """Run all tests."""
    print("\n" + "=" * 70)
    print("  STEP 5.2 API ENDPOINTS TESTING (AUTOMATED)")
    print("=" * 70)
    
    # Wait for server
    if not wait_for_server():
        print("\n✗ Server is not responding. Make sure it's running on http://127.0.0.1:8000")
        return
